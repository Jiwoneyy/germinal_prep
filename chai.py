...
def run_chai(
    ...
    constraint_path_ = None
    constraint_path = None

    if cdr3_idx is not None and hotspot_residue is not None:
        constraint = True
        constraint_path_ = Path(__file__).with_name("chai.restraints")
        rests_df = pd.read_csv(constraint_path_)
        ...
        rests_df.to_csv(constraint_path_, index=False)
    else:
        constraint = False
    ...
    candidates = run_inference(
        ...
        constraint_path = constraint_path_ if constraint else None,  # Path to restraints file
    )
